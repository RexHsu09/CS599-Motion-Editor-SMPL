...
class MotionEditorPipeline(DiffusionPipeline):
    ...
    def _fuse_guidance_maps(self, depth_map, normal_map, semantic_map, dwpose_map, device, dtype):
        """
        Fuse 4 guidance maps into a 12-channel tensor, then project to 3 channels.
        
        Args:
            depth_map: [B, F, C, H, W] or [F, C, H, W] or [F, 1, H, W] - video format
            normal_map: [B, F, C, H, W] or [F, C, H, W] or [F, 1, H, W]
            semantic_map: [B, F, C, H, W] or [F, C, H, W] or [F, 1, H, W]
            dwpose_map: [B, F, C, H, W] or [F, C, H, W] or [F, 1, H, W]
        ...
        for name, m in zip(map_names, [depth_map, normal_map, semantic_map, dwpose_map]):
            if m is None:
                raise ValueError(f"Guidance fusion requires all 4 maps, but {name} is None")
            
            m = m.to(device=device, dtype=dtype)
            
            # Handle shape variations: convert [B, F, C, H, W] or [B, C, F, H, W] to [F, C, H, W]
            if len(m.shape) == 5:
                # Could be [B, F, C, H, W] or [B, C, F, H, W]
                # Assume [B, F, C, H, W] format from dataset
                if m.shape[0] == 1:  # batch size is 1
                    m = m.squeeze(0)  # [F, C, H, W]
                else:
                    raise ValueError(f"{name} map has unexpected batch dimension: {m.shape}")
            
            # Now m should be [F, C, H, W]
            if len(m.shape) == 4:
                # Check if it's [F, C, H, W] or [C, F, H, W]
                # Assume [F, C, H, W] - if F > C and C is small (1 or 3), this should be correct
                pass
            elif len(m.shape) == 3:  # [F, H, W]
                m = m.unsqueeze(1)  # [F, 1, H, W]
            else:
                raise ValueError(f"{name} map has unexpected shape: {m.shape}")
            
            # Ensure 3 channels
            if m.shape[1] == 1:
                m = m.repeat(1, 3, 1, 1)  # [F, 3, H, W]
            elif m.shape[1] == 3:
                pass  # Already 3 channels
            else:
                raise ValueError(f"{name} map has {m.shape[1]} channels, expected 1 or 3. Shape: {m.shape}")
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if use_guidance_fusion:
            ...
            print("[ControlNet] Using multi-layer guidance fusion mode (depth, normal, semantic, dwpose)")
            controlnet_target_guidance = self._fuse_guidance_maps(
                depth_map, normal_map, semantic_map, dwpose_map, device, self.controlnet.dtype
            )
        else:
            # Legacy skeleton mode
            print("[ControlNet] Using legacy skeleton mode")
            if skeleton is None:
                raise ValueError("Either skeleton or all 4 guidance maps (depth, normal, semantic, dwpose) must be provided")
            controlnet_target_guidance = torch.unsqueeze(skeleton[-1], dim=0)
        
        controlnet_batch_size = 1
        if isinstance(self.controlnet, ControlNetModel):
            images = self.prepare_image(
                image=controlnet_target_guidance,
                width=width,
                height=height,
