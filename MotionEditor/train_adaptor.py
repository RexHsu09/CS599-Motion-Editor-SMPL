...
def fuse_guidance_maps(guidance_proj, source_conditions, guidance_types, device, dtype):
    guidance_maps = []
    for guidance_type in guidance_types:
        if guidance_type not in source_conditions:
            raise KeyError(f"Missing guidance type {guidance_type} in source_conditions")
        guidance_maps.append(source_conditions[guidance_type].to(device=device, dtype=dtype))

    fused_guidance = torch.cat(guidance_maps, dim=2)
    batch_size, video_length, _, _, _ = fused_guidance.shape
    fused_guidance = rearrange(fused_guidance, "b f c h w -> (b f) c h w")

    # Match the projection module's dtype (it's fp32 because it's trainable)
    proj_dtype = next(guidance_proj.parameters()).dtype
    fused_guidance = fused_guidance.to(dtype=proj_dtype)

    fused_guidance = guidance_proj(fused_guidance)
    fused_guidance = rearrange(fused_guidance, "(b f) c h w -> b f c h w", b=batch_size, f=video_length)
...
def main(
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    guidance_fusion_proj = unet.controlnet_adapter.guidance_fusion_proj
        # Freeze vae, text_encoder, and unet
    vae.requires_grad_(False)
    text_encoder.requires_grad_(False)
    unet.requires_grad_(False)
    controlnet.requires_grad_(False)
    guidance_fusion_proj.requires_grad_(True)
    ...
    guidance_fusion_proj.to(accelerator.device, dtype=torch.float32)
    ...
    if resume_from_checkpoint:
        ...
        accelerator.load_state(os.path.join(output_dir, path), strict=False)
    ...
    accelerator.load_state(resume_from_one_stage_checkpoint, strict=False)
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(input_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                guidance_types = list(input_data.condition) if isinstance(input_data.condition, (list, tuple)) or OmegaConf.is_list(input_data.condition) else [input_data.condition]
                source_skeleton = fuse_guidance_maps(
                    guidance_fusion_proj,
