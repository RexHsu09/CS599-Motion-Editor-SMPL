...
def main(
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    for step, batch in enumerate(input_dataloader):
        ...
        # Extract all 4 guidance maps for inference
        target_depth_map = batch["target_conditions"].get("depth", None)
        target_normal_map = batch["target_conditions"].get("normal", None)
        target_semantic_map = batch["target_conditions"].get("semantic_map", None)
        target_dwpose_map = batch["target_conditions"].get("openposefull", None)
        
        if target_depth_map is not None:
            target_depth_map = target_depth_map.to(weight_dtype)
        if target_normal_map is not None:
            target_normal_map = target_normal_map.to(weight_dtype)
        if target_semantic_map is not None:
            target_semantic_map = target_semantic_map.to(weight_dtype)
        if target_dwpose_map is not None:
            target_dwpose_map = target_dwpose_map.to(weight_dtype)
        ...
        for idx, prompt in enumerate(validation_data.prompts):
            ...
            sample = validation_pipeline(prompts,
                                         ...
                                         normal_map=target_normal_map,
                                         semantic_map=target_semantic_map,
                                         dwpose_map=target_dwpose_map,
                                         source_masks=None,
                                         target_masks=None,
